...
namespace gates 
{
    ...
    void matrix_multiply_2x2(const std::vector<std::vector<Coeff>>& g1, const std::vector<std::vector<Coeff>>& g2, std::vector<std::vector<Coeff>>& g3) ;
    void controlled_pauli_Z(struct state_vector & sv, int cbit, int tbit) ;
    void print_matrix(const std::vector < std::vector < Coeff >> & matrix) ;
    void kronecker_sub(const std::vector < std::vector < Coeff >> & m1,
    const std::vector < std::vector < Coeff >> & m2, std::vector < std::vector < Coeff >> & m3);
    void kronecker_buff(const std::vector < std::vector < Coeff >> & gate, std::vector < std::vector < Coeff >> & res, int n_qbits, int tbit) ;
